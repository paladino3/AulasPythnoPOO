...
class Table:
    ...
    def __init__(self, players):
        self.players = [players(name,Hand()) for name in players]
        self.deck = Deck()
        self.rounds = 0


    def deal_cards(self):
        self.deck.shuffle()#embaralhamento
        self.deck.setup_hands(self.players)
        for player in self.players:
            player.show_hand()


    def play_once(self, tied = None):#jogar rodada
        if tied is None:
            self.count_round()
        collection = Pot()
    ...
    def count_round(self):
        self.rounds+=1
        print_underline("Começando rodada {}".format(self.rounds), "=")
...
class Pot:
    ...
    def show_pot(self):
        for player, card in zip(self.players, self.cards):
            print("{} jogou {}.".format(player.name, card))
    ...
    @property
    def tied(self):
        for card, player in zip(self.cards, self.players):#retorna uma lista de tupple
            if card.value == self.best:
                yield player #funciona como um return

def print_underline(string, line):
    print("\n{}\n{}".format(string,line*len(string)))

table = Table(["Wesley", "Sarah", "Pedro","Davi"])
table.deal_cards()
table.play_all()
